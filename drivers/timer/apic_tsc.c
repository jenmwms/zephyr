...
int sys_clock_driver_init(const struct device *dev)
{
	...
	/* When in this mode, we "own" the TSC, so make sure all cores
	 * zero any adjustment values the firmware might have put
	 * there.  We do the same in smp_timer_init().  This
	 * guarantees cycle-exact synchronization as long as the
	 * hardware can do that.  Hotplug and other multi-socket
	 * complications affect this and will need more elaborate
	 * support if we get there.  This is good for any
